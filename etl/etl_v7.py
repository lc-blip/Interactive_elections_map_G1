...
EXCEL_FILE_RESULTS = '~/Documentos/work_dir/Interactive_elections_map_G1/dataset/mapa_1_resultados_modificado.xlsx'
EXCEL_FILE_MANDATES = '~/Documentos/work_dir/Interactive_elections_map_G1/dataset/mapa_2_perc_mandatos_modificado.xlsx'
...
def resolve_detailed_names(df_votes, df_source):
    ...
    for idx, row in df_source.iterrows():
        ...
        if col_gce and pd.notna(row[col_gce]):
            ...
            matches = re.findall(r'\[(.*?)\]', raw_text)
            if matches:
                 real_name_map[(conc_id, 'GCE')] = ", ".join(matches)
            elif len(raw_text) > 1:
                 real_name_map[(conc_id, 'GCE')] = raw_text
...
def run_etl():
    ...
    # 5. Gravar na BD
    print(f"Gravando base de dados: {DB_FILE}")
