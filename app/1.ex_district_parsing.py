...
# ---------------------- Config ----------------------
SQLITE_DB   = "../scripts/geom_db/geometry.db"

# Tabelas e colunas
T_DISTRICTS = "districts"
D_NAME      = "name"
#D_REGION    = "region" nao temos isto, faz sentido ter???
D_GEOM      = "geom"
"""
...
class App:
    ...
    def draw_districts(self):
        ...
        data = fetch_districts()  # [(name, polys)]
        if not data:
        ...
        for name, polys in data:
            for rings in polys:
                ...
                if pid:
                    # clique no distrito (por agora só print)
                ...
                # buracos (interior rings) — pinta com background
                for hole in rings[1:]:
                    pts_h = []
                    for x, y in hole:
                        X, Y = project(x, y)
